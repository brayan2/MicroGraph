...
async function introspectPersonalisation() {
    // First find the exact model name. The user said "personalisation page model"
    const listQuery = `query { __schema { types { name } } }`;
    ...
    const typeName = listJson.data.__schema.types.find(t => t.name.toLowerCase().includes('personalisationpage'))?.name;

    if (!typeName) {
        console.log("Could not find PersonalisationPage model. Available similar types:",
            listJson.data.__schema.types.filter(t => t.name.toLowerCase().includes('person')).map(t => t.name));
        return;
    }
...
introspectPersonalisation().catch(console.error);
