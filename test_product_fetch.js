...
async function main() {
    console.log("Testing product fetch with variants (same query as frontend)...\n");

    // Test with a known product slug
  ...
  `, { slug: 'ultimate-tee', segmentName: null });

    if (data?.products?.[0]) {
        const product = data.products[0];
        console.log("Product:", product.title);
        console.log("productPrice:", product.productPrice);
        console.log("productPrice?.price:", product.productPrice?.price);
        console.log("variants:", product.variants);
        console.log("\nWhat the frontend would display:");
        console.log(`  variants?.[0]?.productPrice?.price: ${product.variants?.[0]?.productPrice?.price}`);
        console.log(`  productPrice?.price: ${product.productPrice?.price}`);
        console.log(`  Final: ${product.variants?.[0]?.productPrice?.price ?? product.productPrice?.price ?? 0}`);
    } else {
