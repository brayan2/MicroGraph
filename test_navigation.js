...
async function test(locale) {
    console.log(`Testing navigation for locale: ${locale}`);
    const query = `
    query GetNavigation($locale: [Locale!]!) {
      navigations(first: 1) {
        id
        title
        navItmes(locales: $locale) {
          id
          label
          mulitpleLinks(locales: $locale) {
            ... on Link {
              __typename
              id
              url
  ...
  `;
    const data = await graphqlRequest(query, { locale: [locale] });
    console.log(JSON.stringify(data, null, 2));
}

test('en').then(() => test('de'));
