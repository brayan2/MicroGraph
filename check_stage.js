...
async function main() {
    // Query both DRAFT and PUBLISHED
    const queryDraft = `
    query {
      products(where: { productSlug: "blue-tshirt" }, first: 1, stage: DRAFT) {
        title
  ...
  `;
    ...
    query {
      products(where: { productSlug: "blue-tshirt" }, first: 1, stage: PUBLISHED) {
        title
        relatedProducts { product { id title } }
      }
    }
  `;
    ...
    let res = await fetch(endpoint, {
        ...
        body: JSON.stringify({ query: queryDraft })
    });
    let json = await res.json();
    console.log(JSON.stringify(json.data?.products?.[0]?.relatedProducts, null, 2));

    console.log("\nChecking PUBLISHED:");
    res = await fetch(endpoint, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ query: queryPublished })
    });
    json = await res.json();
    console.log(JSON.stringify(json.data?.products?.[0]?.relatedProducts, null, 2));
}
