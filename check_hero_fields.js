...
async function introspect() {
    // Check Query type for HeroSlider fields
    const query = `query {
    __type(name: "Query") {
      fields {
        name
    ...
    // Find fields that contain "hero" or "slider"
    const queryFields = json.data.__type.fields;
    const heroFields = queryFields.filter(f =>
        f.name.toLowerCase().includes('hero') ||
        f.name.toLowerCase().includes('slider')
    );

    console.log("HeroSlider-related query fields:");
    console.log(JSON.stringify(heroFields, null, 2));
}
