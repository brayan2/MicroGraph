...
async function graphqlRequest(query, variables, ignoreErrors = false) {
...
async function main() {
    ...
    console.log("Attempting to unpublish and republish landing page...");

    // Try unpublishing first
    console.log("1. Unpublishing...");
    await graphqlRequest(`
    mutation {
      unpublishLandingPage(where: { id: "${landingPageId}" }, from: PUBLISHED) { id }
    }
  `, null, true);
  ...
  `);
    ...
    if (result.data) {
        ...
        console.log("Publishing still failing. Let me check what's actually in the landing page...\n");

        const lpCheck = await graphqlRequest(`
      query {
        landingPages(where: { id: "${landingPageId}" }, stage: DRAFT) {
          id
          title
          sections {
            __typename
    ...
    `);
        ...
        console.log(JSON.stringify(lpCheck.data, null, 2));
    }
}
