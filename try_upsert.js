...
async function main() {
  ...
  `);
    ...
    `);
        ...
        if (result) {
            const count = result.updateProduct?.relatedProducts?.product?.length || 0;
            console.log(`  Mutation returned: ${count} related`);
  ...
  `);
    ...
    for (const p of verify.products) {
        console.log(`  ${p.title}: ${p.relatedProducts?.product?.length || 0} related`);
    }
}
