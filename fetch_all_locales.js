...
async function fetchAllLocales() {
    const locales = ['en', 'de', 'fr', 'es']; // Some common locales
    const results = {};

    for (const locale of locales) {
        const query = `
          query GetNavigation($locale: [Locale!]!) {
            navigations(stage: DRAFT) {
              id
              title
        ...
        const res = await fetch(endpoint, {
            ...
            body: JSON.stringify({ query, variables: { locale: [locale] } })
        });
        const json = await res.json();
        results[locale] = json.data;
    }
    console.log(JSON.stringify(results, null, 2));
}

fetchAllLocales().catch(console.error);
