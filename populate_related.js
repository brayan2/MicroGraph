  ...
  `);
    ...
    for (const product of products) {
        if (product.relatedProducts.length > 0) {
            console.log(`✓ ${product.title} already has ${product.relatedProducts.length} related products`);
            continue;
        }
        ...
        console.log(`> Assigning ${selected.length} related products to: ${product.title}`);
      ...
      mutation {
        updateProduct(
          where: { id: "${product.id}" }
          data: { relatedProducts: { connect: [${connectString}] } }
        ) { id }
      }
    `);
