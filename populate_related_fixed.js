...
async function main() {
  ...
  for (const product of products) {
    ...
    if (product.relatedProducts && product.relatedProducts.product.length >= 3) {
      console.log(`✓ ${product.title} has ${product.relatedProducts.product.length} related products`);
      continue;
    }
    ...
    // Force replace by using create.
    // Hygraph treats 'create' on a singleton component/basic field as a replacement.
    console.log(`> Assigning related products to: ${product.title}`);
