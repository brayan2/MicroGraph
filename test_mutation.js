...
async function main() {
  ...
  `);
    ...
    // Find Blue Tshirt
    const blueTshirt = products.find(p => p.productSlug === 'blue-tshirt');
    if (!blueTshirt) {
        console.log("Blue Tshirt not found");
        return;
    }

    console.log(`\nBlue Tshirt ID: ${blueTshirt.id}`);

    // Get 3 other product IDs
    const others = products.filter(p => p.id !== blueTshirt.id).slice(0, 3);
    console.log("Will connect to:", others.map(p => p.title).join(', '));

    // Delete existing relatedProducts
    console.log("\nDeleting existing relatedProducts...");
  ...
  `);
    ...
    // Create new with connections  
    const connectIds = others.map(p => `{ id: "${p.id}" }`).join(', ');
    console.log("Creating relatedProducts with connect...");
  ...
  `);
    ...
    if (result) {
        console.log("\nResult:");
        console.log(JSON.stringify(result.updateProduct.relatedProducts, null, 2));
