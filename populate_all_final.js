...
async function main() {
  ...
  `);
    ...
    for (const product of products) {
        ...
        if (currentCount >= 3) {
            console.log(`✓ ${product.title}: Already has ${currentCount} related`);
            continue;
        }

        console.log(`> ${product.title}: Has ${currentCount}, updating...`);
    ...
    `);
        ...
        if (result?.updateProduct?.relatedProducts?.product?.length > 0) {
            await graphqlRequest(`mutation { publishProduct(where: { id: "${product.id}" }, to: PUBLISHED) { id } }`);
            console.log(`  ✓ Now has ${result.updateProduct.relatedProducts.product.length} related`);
        } else {
            console.log(`  ✗ Failed`);
        }
