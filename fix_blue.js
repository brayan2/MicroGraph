
const endpoint = "https://api-ap-southeast-2.hygraph.com/v2/cmfc5wvn401kl08v2so36gw1l/master";
const token = "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6ImdjbXMtbWFpbi1wcm9kdWN0aW9uIn0..cbNOHhSkxNysJqx9gsY3pFFzoPkJOPx1AY_vR4EJJWRto4qG4RN-1y2YpeVR24aSGZ5rRMUjVHRfgFOe-xgdiVPCHQw36xrbzuTocGtWOr6eSPMuNm7uZDEuiWNdiGTbxMuzh0OvGN8tXLGcMsVUw3w6TFcLP3Rv5xAaXSISeN5OJL6_Cq7oT0SP9SP7KbcD1sVFYXCoC9DTchHbJYNd0I6CecOUH3Sy8mzuXr6YQxJ73FtoXQJwwefpY4Xg3rTLeOCl7JrAQ85tWe8qmMCuFEQdUXU2kMY9Ok9nVIQDL7ZdoLpftW89giihrJivqv6CTOBADV8XUu5VLMeXbLuSBRb4xmcbDOIX0_7THK_gZAolKQ3KZdbxgrNd1246Mv1P472SwFLCc0Hs3jpalxGBCk9Za4DtHEEGpmb96u84VCpna-a7g2LoHiZZMcOZYXMpJi1iRwVJKk_1Ij87_SCM9OFme9rWG0AM_-3qI0wSbq3O8oFfhLVqU8kFu7yYa5n-XO8VqzuFsVVnc1V0UukJ9Mv1KrOt7Y_DgWebT0DO5WcMfr9wCijcutP3jUFjVKQYE7qSX3VsigJngPdDeY0vDDujWK4pTdvg9V14MpQPKUcVShpJU0gweTmOboVZp6VGHwxVAN5dhUrdOiLawnPRhSyVHSZUt-lTovHUY3iaZgQ";

async function graphqlRequest(query, variables) {
    const res = await fetch(endpoint, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`
        },
        body: JSON.stringify({ query, variables })
    });
    const json = await res.json();
    if (json.errors) {
        console.error("GraphQL Error:", JSON.stringify(json.errors, null, 2));
        return null;
    }
    return json.data;
}

const sleep = (ms) => new Promise(r => setTimeout(r, ms));

async function main() {
    // 1. Get Blue Tshirt and others
    const data = await graphqlRequest(`
    query {
      products(first: 100) {
        id
        title
        productSlug
      }
    }
  `);

    const blueTshirt = data.products.find(p => p.productSlug === 'blue-tshirt');
    const others = data.products.filter(p => p.id !== blueTshirt.id).slice(0, 3);
    const connectIds = others.map(p => `{ id: "${p.id}" }`).join(', ');

    console.log(`Blue Tshirt ID: ${blueTshirt.id}`);
    console.log(`Connecting to: ${others.map(p => p.title).join(', ')}`);

    // 2. Delete existing RelatedProduct component
    console.log("Deleting existing component...");
    await graphqlRequest(`
    mutation {
      updateProduct(
        where: { id: "${blueTshirt.id}" }
        data: { relatedProducts: { delete: true } }
      ) { id }
    }
  `);

    await sleep(1000);

    // 3. Create new component with connections
    console.log("Creating new component...");
    const result = await graphqlRequest(`
    mutation {
      updateProduct(
        where: { id: "${blueTshirt.id}" }
        data: {
          relatedProducts: {
            create: {
              product: { connect: [${connectIds}] }
            }
          }
        }
      ) { 
        relatedProducts { product { id title } }
      }
    }
  `);

    if (result) {
        console.log("Creation successful!");
        console.log("Related:", result.updateProduct.relatedProducts.product.map(p => p.title));

        // 4. Publish
        console.log("Publishing...");
        await graphqlRequest(`mutation { publishProduct(where: { id: "${blueTshirt.id}" }, to: PUBLISHED) { id } }`);

        await sleep(2000); // Wait for publish

        // 5. Verify PUBLISHED stage
        console.log("Verifying PUBLISHED stage...");
        const verify = await graphqlRequest(`
      query {
        products(where: { id: "${blueTshirt.id}" }, stage: PUBLISHED) {
          title
          relatedProducts { product { title } }
        }
      }
    `);

        console.log("Published Data:", JSON.stringify(verify.products[0].relatedProducts, null, 2));
    }
}

main().catch(console.error);
