...
async function main() {
    ...
    // Query from DRAFT stage to get the latest
    const data = await graphqlRequest(`
    query {
      products(first: 100, stage: DRAFT) {
        id
        title
        relatedProducts {
          product { id title }
  ...
  `);
    ...
    console.log("DRAFT stage data:");
    for (const p of data.products) {
        const count = p.relatedProducts?.product?.length || 0;
        console.log(`  ${p.title}: ${count} related`);
    }

    console.log("\nPublishing all products...");

    for (const p of data.products) {
        await graphqlRequest(`mutation { publishProduct(where: { id: "${p.id}" }, to: PUBLISHED) { id } }`);
        console.log(`  Published: ${p.title}`);
        await sleep(100);
    }

    console.log("\nQuerying PUBLISHED stage...");

    const pubData = await graphqlRequest(`
    query {
      products(first: 100, stage: PUBLISHED) {
        id
        title
        relatedProducts {
          product { id title }
  ...
  `);
    ...
    console.log("PUBLISHED stage data:");
    for (const p of pubData.products) {
        const count = p.relatedProducts?.product?.length || 0;
        console.log(`  ${p.title}: ${count} related`);
    }
}
