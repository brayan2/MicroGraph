...
async function main() {
    ...
    const data = await graphqlRequest(`query { products(first: 100) { id title relatedProducts { product { id } } } }`);
    ...
    for (const product of products) {
        if (product.title === "Blue Tshirt") {
            console.log(`✓ Blue Tshirt: Skipping (already fixed)`);
            continue;
        }

        // Check if checks out (3 related)
        if (product.relatedProducts?.product?.length >= 3) {
            console.log(`✓ ${product.title}: Already has ${product.relatedProducts.product.length} related`);
            // We still need to publish to make sure it's live
        ...
        const connectIds = selected.map(id => `{ id: "${id}" }`).join(', ');
