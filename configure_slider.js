...
async function main() {
    console.log("Configuring HeroSlider...");

    // 1. Fetch Landing Page 'home-v3' and Products
    const query = `
    query GetInfo {
      landingPages(where: { pageSlug: "home-v3" }, first: 1) {
        ...
        sections {
          ...
          ... on Cta { id }
          ... on ProductGrid { id }
          ... on BlogTeasersSection { id }
          ... on RemoteReviewsSection { id }
        }
      }
      products(first: 5) {
        id
  ...
  `;
    ...
    // Find the slider
    let slider = landingPage.sections.find(s => s.__typename === 'HeroSlider');
    if (!slider) throw new Error("HeroSlider not found. Please create it manually first.");

    console.log(`Found Slider: ${slider.id}`);

    // 2. Update Slider Content (Add Products)
    // We use 'set' to replace any existing items so we don't duplicate if run multiple times
    console.log("Updating slider items...");
  ...
  `;
    ...
    // 3. Reorder Sections (Move Slider to Top)
    console.log("Reordering sections...");

    // Filter out the slider from its current position
    const otherSections = landingPage.sections.filter(s => s.id !== slider.id);

    // Create new ordered list: [Slider, ...Others]
  ...
  `;
    ...
    await graphqlRequest(updateLpMutation, { id: landingPage.id, sections: orderedSections });
    await graphqlRequest(`mutation { publishLandingPage(where: { id: "${landingPage.id}" }, to: PUBLISHED) { id } }`);

    console.log("Success: Slider populated and moved to top!");
}
