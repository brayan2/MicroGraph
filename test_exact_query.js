...
async function main() {
    ...
    query GetProductWithVariants($slug: String!) {
      productBySlug: products(where: { productSlug: $slug }, first: 1) {
        id
        title
        relatedProducts {
          product {
      ...
      productById: products(where: { id: "cmfcgrlf58hwb072qlz4q2y8g" }, first: 1) {
        id
        title
        relatedProducts {
          product {
  ...
  `;
    ...
    const res = await fetch(endpoint, {
        ...
        body: JSON.stringify({ query, variables: { slug: "blue-tshirt", segmentName: null } })
    });
