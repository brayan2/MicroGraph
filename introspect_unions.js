...
async function introspectUnions() {
    const unions = ["NavigationnavItemsUnion", "NavItemsubNavsUnion", "navItemTarget_1oeid"];
    const results = {};
    ...
    `;
        const res = await fetch(endpoint, {
            ...
            body: JSON.stringify({ query, variables: { name: union } })
        });
        const json = await res.json();
        results[union] = json.data.__type;
    }

    console.log(JSON.stringify(results, null, 2));
}

introspectUnions().catch(console.error);
