...
async function main() {
    ...
    const query1 = `
    query {
      products(where: { productSlug: "blue-tshirt" }, first: 1) {
        title
        relatedProducts {
          product { id title }
        }
  ...
  `;
    ...
    for (const [label, query] of [['Default', query1], ['With first:10', query2], ['DRAFT stage', query3]]) {
