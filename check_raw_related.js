...
async function main() {
    // Get all RelatedProduct entries
    ...
    if (json.errors) {
        ...
        console.log("All RelatedProduct entries:");
        json.data.relatedProducts.forEach(rp => {
            console.log(`  ID: ${rp.id}`);
            console.log(`    similarProducs: ${rp.similarProducs}`);
            console.log(`    product count: ${rp.product?.length || 0}`);
            rp.product?.forEach(p => console.log(`      - ${p.title}`));
        });
    }
}
