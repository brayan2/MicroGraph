...
async function findAllNav() {
    ...
    const result = json.data.__schema.types.filter(t => t.name && (t.name.toLowerCase().includes('nav') || t.name.toLowerCase().includes('menu')));
    console.log(JSON.stringify(result, null, 2));
}

findAllNav().catch(console.error);
