...
async function introspect() {
    const models = [
        "BlogAuthor", "BlogPost", "LandingPage", "Navigation", "Product",
        "ProductCategory", "ProductReviews", "ProductSeller", "Segment",
        "BlogTeasersSection", "ClothesVariants", "Cta", "HeroSection", "Link",
        "NavItem", "ProductAttributes", "ProductPrice", "ProductGrid",
        "RelatedProduct", "RemoteReviewsSection", "SampleComponent", "Seo", "ShoesVariant"
    ];
    ...
    `;
        const res = await fetch(endpoint, {
            ...
            body: JSON.stringify({ query, variables: { name: model } })
        });
        const json = await res.json();
        results[model] = json.data.__type;
    }
