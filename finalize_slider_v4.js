...
async function run() {
    console.log("Fetching home-v3 and products...");
  ...
  `);
    ...
    // Identify sliders to delete (duplicates)
    const slidersToDelete = lp.sections
        .filter(s => s.__typename === "HeroSlider")
        .map(s => ({ HeroSlider: { id: s.id } }));

    console.log(`Found ${slidersToDelete.length} existing slider(s). Preparing to delete and recreate at top.`);

    const mutation = `
    mutation FinalizeSlider($id: ID!, $deleteIds: [LandingPagesectionsUnionWhereUniqueInput!], $connectProducts: [ProductWhereUniqueInput!]) {
      updateLandingPage(
        where: { id: $id }
        data: {
          sections: {
  ...
  `;
    ...
    const variables = {
        id: lp.id,
        deleteIds: slidersToDelete,
        connectProducts: productIds.map(id => ({ id }))
    };

    const updateRes = await graphqlRequest(mutation, variables);
    if (updateRes) {
        console.log("Success! Publishing LandingPage...");
