
const endpoint = "https://api-ap-southeast-2.hygraph.com/v2/cmfc5wvn401kl08v2so36gw1l/master";
const token = "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6ImdjbXMtbWFpbi1wcm9kdWN0aW9uIn0..cbNOHhSkxNysJqx9gsY3pFFzoPkJOPx1AY_vR4EJJWRto4qG4RN-1y2YpeVR24aSGZ5rRMUjVHRfgFOe-xgdiVPCHQw36xrbzuTocGtWOr6eSPMuNm7uZDEuiWNdiGTbxMuzh0OvGN8tXLGcMsVUw3w6TFcLP3Rv5xAaXSISeN5OJL6_Cq7oT0SP9SP7KbcD1sVFYXCoC9DTchHbJYNd0I6CecOUH3Sy8mzuXr6YQxJ73FtoXQJwwefpY4Xg3rTLeOCl7JrAQ85tWe8qmMCuFEQdUXU2kMY9Ok9nVIQDL7ZdoLpftW89giihrJivqv6CTOBADV8XUu5VLMeXbLuSBRb4xmcbDOIX0_7THK_gZAolKQ3KZdbxgrNd1246Mv1P472SwFLCc0Hs3jpalxGBCk9Za4DtHEEGpmb96u84VCpna-a7g2LoHiZZMcOZYXMpJi1iRwVJKk_1Ij87_SCM9OFme9rWG0AM_-3qI0wSbq3O8oFfhLVqU8kFu7yYa5n-XO8VqzuFsVVnc1V0UukJ9Mv1KrOt7Y_DgWebT0DO5WcMfr9wCijcutP3jUFjVKQYE7qSX3VsigJngPdDeY0vDDujWK4pTdvg9V14MpQPKUcVShpJU0gweTmOboVZp6VGHwxVAN5dhUrdOiLawnPRhSyVHSZUt-lTovHUY3iaZgQ";

const client = {
    request: async (query, variables = {}) => {
        const res = await fetch(endpoint, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${token}`
            },
            body: JSON.stringify({ query, variables })
        });
        const json = await res.json();
        if (json.errors) {
            console.error(JSON.stringify(json.errors, null, 2));
            throw new Error(JSON.stringify(json.errors, null, 2));
        }
        return json.data;
    }
};

async function updateLinks() {
    console.log("Updating Homepage Links (Debug Mode - Hero Only)...");

    // 1. Find correct CTAs
    const data = await client.request(`
    query {
      landingPages(where: { pageSlug: "home-v3" }) {
        id
        sections {
          __typename
          ... on Cta {
            id
            ctaTxt
          }
        }
      }
    }
  `);

    const lp = data.landingPages[0];
    const sections = lp.sections;
    const heroCta = sections.find(s => s.ctaTxt === "Welcome to Micro-Store");

    if (!heroCta) {
        console.log("Hero CTA not found.");
        return;
    }

    console.log(`Hero CTA ID: ${heroCta.id}`);

    const updates = [];
    updates.push({
        Cta: {
            where: { id: heroCta.id },
            data: {
                buttonLink: { create: { url: "/collection", openNewTab: false } }
            }
        }
    });

    console.log("Sending Update Mutation...");

    await client.request(`
    mutation UpdateLandingPageSections($id: ID!, $sectionsUpdate: [LandingPagesectionsUnionUpdateWithNestedWhereUniqueAndPositionInput!]!) {
      updateLandingPage(
        where: { id: $id }
        data: {
          sections: {
            update: $sectionsUpdate
          }
        }
      ) {
        id
        sections {
            ... on Cta { ctaTxt buttonLink { url } }
        }
      }
    }
  `, {
        id: lp.id,
        sectionsUpdate: updates
    });

    console.log("Publishing...");
    await client.request(`mutation { publishLandingPage(where: { id: "${lp.id}" }, to: PUBLISHED) { id } }`);

    console.log("✅ Hero Link updated!");
}

updateLinks().catch(console.error);
